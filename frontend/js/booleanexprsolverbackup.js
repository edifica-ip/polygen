...
function stringify(node) {
    ...
    switch (node.type) {
        ...
        case 'AND': 
            // Add parens to preserve precedence when stringifying
            let lAnd = node.left.type === 'OR' ? `(${stringify(node.left)})` : stringify(node.left);
            let rAnd = node.right.type === 'OR' ? `(${stringify(node.right)})` : stringify(node.right);
            // Replaced the * with a standard dot
            return `${lAnd}.${rAnd}`;
        case 'OR': 
...
// Absorption Law (AND): A * (A+B) = A  |  (A+B) * A = A
        
        // Case 1: Left is A, Right contains A as an addend
        if (node.right.type === 'OR' && hasAddend(node.right, node.left)) {
            state.changed = true;
            state.ruleUsed = "Absorption Law (A * (A+...) = A)";
            return node.left;
        }
        // Case 2: Right is A, Left contains A as an addend
        if (node.left.type === 'OR' && hasAddend(node.left, node.right)) {
            state.changed = true;
            state.ruleUsed = "Absorption Law ((...+A) * A = A)";
            return node.right;
        }
...
// Redundancy Law (AND): A * (!A + B) = A * B
        if (node.right.type === 'OR') {
            if (isComplement(node.left, node.right.left)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (A * (!A + B) = A * B)";
                return { type: 'AND', left: node.left, right: node.right.right };
            }
            if (isComplement(node.left, node.right.right)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (A * (B + !A) = A * B)";
                return { type: 'AND', left: node.left, right: node.right.left };
            }
        }
        
        // Redundancy Law (AND) Reverse: (!A + B) * A = B * A
        if (node.left.type === 'OR') {
            if (isComplement(node.right, node.left.left)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law ((!A + B) * A = B * A)";
                return { type: 'AND', left: node.left.right, right: node.right };
            }
            if (isComplement(node.right, node.left.right)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law ((B + !A) * A = B * A)";
                return { type: 'AND', left: node.left.left, right: node.right };
            }
        }
                    ...
                    if (uniqueConsensus.length > 0) {
                        ...
                        for (let k = 0; k < flatOrConsensus.length; k++) {
                            ...
                            if (hashNode(flatOrConsensus[k]) === consensusHash) {
                                ...
                                state.changed = true;
                                state.ruleUsed = "Consensus Theorem (XY + !XZ + YZ = XY + !XZ)";
...
// Absorption Law (OR): A + A*B = A  |  A*B + A = A
        // Case 1: Left is A, Right is (A * B) or (B * A)


        // Case 1: Left is A, Right contains A as a factor
        if (node.right.type === 'AND' && hasFactor(node.right, node.left)) {
            state.changed = true;
            state.ruleUsed = "Absorption Law (A + A*... = A)";
            return node.left;
        }
        // Case 2: Right is A, Left contains A as a factor
        if (node.left.type === 'AND' && hasFactor(node.left, node.right)) {
            state.changed = true;
            state.ruleUsed = "Absorption Law (...*A + A = A)";
            return node.right;
        }
...
// Redundancy Law (OR): A + !A*B = A + B
        if (node.right.type === 'AND') {
            if (isComplement(node.left, node.right.left)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (A + !A*B = A + B)";
                return { type: 'OR', left: node.left, right: node.right.right };
            }
            if (isComplement(node.left, node.right.right)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (A + B*!A = A + B)";
                return { type: 'OR', left: node.left, right: node.right.left };
            }
        }
        
        // Redundancy Law (OR) Reverse: !A*B + A = B + A
        if (node.left.type === 'AND') {
            if (isComplement(node.right, node.left.left)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (!A*B + A = B + A)";
                return { type: 'OR', left: node.left.right, right: node.right };
            }
            if (isComplement(node.right, node.left.right)) {
                state.changed = true;
                state.ruleUsed = "Redundancy Law (B*!A + A = B + A)";
                return { type: 'OR', left: node.left.left, right: node.right };
            }
        }
    ...
    if (node.type === 'NOT') {
        ...
        if (node.child.type === 'NOT') {
            state.changed = true;
            state.ruleUsed = "Double Negation Law (!!A = A)";
...
function addStepToList(rule, expr, isFinal = false) {
    ...
    li.innerHTML = `
        <span class="rule-name" style="${isFinal ? 'color: #28a745; font-weight:bold;' : ''}">${rule}</span>
