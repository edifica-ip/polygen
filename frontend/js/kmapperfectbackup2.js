...
function updateMap() {
    ...
    // Handle Edge Cases
    if (targets.length === 0) {
        output.innerText = mode === 'SOP' ? 'Σ()' : 'Π()';
        globalResult.value = mode === 'SOP' ? '0' : '1';
        globalSteps.innerHTML += `<div class="step-item">No cells selected.</div>`;
        return;
    }

    if (targets.length === maxMinterms) {
        const notation = mode === 'SOP' ? `Σ(${targets.join(',')})` : `Π(${targets.join(',')})`;
        output.innerText = notation;
        globalResult.value = mode === 'SOP' ? '1' : '0';
        ...
        globalSteps.innerHTML += `<div class="step-item">Hexadecatet (All ${maxMinterms} cells); → ${mode === 'SOP' ? '1' : '0'}</div>`;
        return;
    }
...
<div class="original">
    ...
    globalSteps.innerHTML += "Therefore:<br>" + notation + " = " + fexpr + "<hr class = kmap-step-divider>";
}
